...
const hookedBodies: Array<{ url: string; body: string }> = [];
let pageHookListenerInstalled = false;
...
export async function deepScanImages(
  ...
  requestPageHookDump();
  await sleep(500);
  collectFromPageHook(addUrl, metadata);
...
function installPageHookListener(): void {
  if (pageHookListenerInstalled) return;
  pageHookListenerInstalled = true;

  window.addEventListener('message', (event) => {
    if (event.source !== window || !event.data || event.data.source !== 'picpick-page-hook') return;
    ...
    if (kind === 'url' && payload?.url) {
      hookedUrls.add(payload.url);
    }

    if (kind === 'body' && payload?.body) {
      hookedBodies.push({ url: payload.url || location.href, body: payload.body });
      if (hookedBodies.length > 30) hookedBodies.shift();
    }
    ...
    if (kind === 'dump') {
      ...
      for (const body of payload?.bodies || []) {
        hookedBodies.push(body);
      }
      while (hookedBodies.length > 30) hookedBodies.shift();
    }
  });
}

function requestPageHookDump(): void {
  window.postMessage({
    source: 'picpick-content',
    kind: 'dump',
  }, '*');
}

function collectFromPageHook(
  ...
  for (const item of hookedBodies) {
    for (const url of extractImageUrlsFromText(item.body, item.url)) {
      addUrl(url, metadata);
    }
  }
}
...
function extractImageUrlsFromText(text: string, baseUrl: string): string[] {
  ...
  for (const pattern of patterns) {
    ...
    while ((match = pattern.exec(text)) !== null) {
      ...
      try {
        const absoluteUrl = new URL(rawUrl, baseUrl || location.href).href;
...
function createScanDebug(): (step: string, data?: unknown) => void {
  ...
  return (step: string, data?: unknown) => {
    if (enabled) {
      console.log(`[PicPick v2.2] ${step}`, data || '');
    }
  };
}
